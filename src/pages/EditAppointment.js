...
const EditAppointment = () => {
  ...
  const {
    ...
    resolver: yupResolver(bookServiceSchema),
    defaultValues: {
      date: dayjs(date),
    },
  });
  ...
  useEffect(() => {
    ...
    getABarberReducer?.res?.success,
    getABarberReducer?.res?.data, // set specific barber in state
    serviceById?.res?.res?.success,
    serviceById?.res?.res?.data, // set specific service by id
    getSchedules?.res?.res?.success,
    getSchedules?.res?.res?.data, // set timeslots
  ]);
    ...
    <NavigationLayout>
      ...
      <Grid container rowSpacing={2} px={{ xs: 2, md: 9 }}>
        ...
        <Grid
          ...
          px={1}
        >
          <UITypogrpahy type="subheading" title="Select Service" />
        </Grid>

        {state?.isServiceSelected && selectedService?.package ? (
